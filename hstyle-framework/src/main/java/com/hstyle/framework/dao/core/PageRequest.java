...
public class PageRequest implements java.io.Serializable {
	...
	public List<Sort> getSort() {
		...
		if(StringUtils.isNotBlank(sortJson)){
			  ...
			  JavaType sortList = jsonMapper.createCollectionType(List.class, Sort.class);
			  sorts = jsonMapper.fromJson(this.sortJson, sortList);
		}
	...
	public boolean isOrderBySetted() {
		return (StringUtils.isNotBlank(orderBy) && StringUtils.isNotBlank(orderDir))||StringUtils.isNotBlank(sortJson);
	}
	...
	public String getOrderSortString() {
		...
		for (Iterator<Sort> it = list.iterator(); it.hasNext();) {
			Sort sort = it.next();
			buffer.append(sort.getProperty() + " " + sort.getDirection()).append(",");
		}
	...
	/**
	 * @param sortJson the sortJson to set
	 */
